        ...
        if column in json_data[person]:
            if pattern is not None:
                if re.search(pattern, json_data[person][column]):
                    to_return.append(json_data[person][column])
            else:
...
def say_column(column):
   return {f'here are the {column}(s)': get_column(column)}

@app.route('/say/<column>/<pattern>')
def say_pattern(column, pattern):
   return {f"here are the {column}(s) according to the pattern '{pattern}'": get_column(column, pattern=pattern)}
